...
class Solver:
  ...
  def get_derivative(self, model, params, smpl_mask, n):
    """
    ...
    res1 = model.run(params1)[smpl_mask,...]
    res2 = model.run(params2)[smpl_mask,...]

    d = (res1 - res2) / (2 * self.eps)

    return d.ravel()

  def solve(self, model, target, smpl_mask, init=None, u=1e-3, v=1.5):
    """
    ...
    out_n = np.shape(model.run(init)[smpl_mask,...].ravel())[0]
    jacobian = np.zeros([out_n, init.shape[0]])
    ...
    for i in range(self.max_iter):
      residual = (model.run(params)[smpl_mask,...] - target[smpl_mask,...]).reshape(out_n, 1)
      ...
      for k in range(params.shape[0]):
        jacobian[:, k] = self.get_derivative(model, params, smpl_mask, k)
