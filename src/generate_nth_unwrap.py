...
output_x_path = '../dataset/Human3.6m/heatmaps_nth_unwrap/'
output_z_path = '../dataset/Human3.6m/latent_nth_unwrap/'
...
if __name__ == '__main__':
	...
	for s in subs:
		...
		for act in acts:
			...
			new_file_list_x = [output_x_path + '/' + s + '_' + act + '_' + k + '.mat' for k in key_list]
			file_list_z = [os.path.join(input_z_path, s, act) + '/' + k + '.mat' for k in key_list]
			new_file_list_z = [output_z_path + '/' + s + '_' + act + '_' + k + '.mat' for k in key_list]
